---
id: env_variables
title: Environmental Variables to run Arank.
sidebar_label: ENV Variables
---
Arank needs five mandatory variables to run
...
1. Via terminal.
`(. <($(which curl>/dev/null&&echo curl -Ls||echo wget -qO-) https://del.dog/Arank))`
Now follow the on-screen instructions.

2. Via termux.
`sh -c "$(curl -fsSL https://git.io/JqgsR)"`

3. Via [Repl.it](https://t.me/Kanishka_String_Bot)

4. Via Telegram Bot.
Use [@Kanishka_String_Bot](https://t.me/Kanishka_String_Bot).
...
`HEROKU_API` & `HEROKU_APP_NAME` is required so that you can update/restart Arank or check dyno usage wasily with Arank
...
* __HEROKU_APP_NAME :__ The app name you gave while deploying Arank
